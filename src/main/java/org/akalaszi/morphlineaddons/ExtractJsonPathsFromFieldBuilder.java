...
/**
 * Command that uses zero or more JSON path expressions to extract values from a
 * JSON object.
 * 
 * 
 ...
 * each path step separated by a '/' slash. JSON arrays are traversed with the
 * '[]' notation.
 * 
 * The result of a path expression is a list of objects, each of which is added
 * to the given record output field.
 * 
 * The path language supports all JSON concepts, including nested structures,
 * records, arrays, etc, as well as a flatten option that collects the
 * primitives in a subtree into a flat list.
 */
public final class ExtractJsonPathsFromFieldBuilder implements CommandBuilder {

	@Override
	public Collection<String> getNames() {
		return Collections.singletonList("extractJsonPathsFromField");
	}

	@Override
	public Command build(Config config, Command parent, Command child, MorphlineContext context) {
		return new ExtractJsonPathsFromField(this, config, parent, child, context);
	}

	private static final class ExtractJsonPathsFromField extends AbstractCommand {

		private final boolean flatten;
		private final Map<String, Collection<String>> stepMap;
		private final String sourceStringField;

		private static final String ARRAY_TOKEN = "[]";

		public ExtractJsonPathsFromField(CommandBuilder builder, Config config, Command parent, Command child,
			...
			this.sourceStringField = getConfigs().getString(config, "sourceStringField");
		...
		protected boolean doProcess(Record inputRecord) {
			JsonNode datum = toJsonNode(inputRecord);
		...
		private JsonNode toJsonNode(Record inputRecord) {
			String jsonString = (String) inputRecord.getFirstValue(sourceStringField);
		...
		private void resolve(JsonNode datum, Record record, String fieldName) {
			...
			if (datum.isObject() || datum.isArray()) {
